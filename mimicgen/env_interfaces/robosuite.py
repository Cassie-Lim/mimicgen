...
class MG_Open(RobosuiteInterface):
    """
    Corresponds to robosuite Open task and variants.
    """
    def get_object_poses(self):
        """
        Gets the pose of each object relevant to MimicGen data generation in the current scene.
        ...
        return dict(
            target=self.get_object_pose(obj_name=self.env.door.root_body, obj_type="body"),
        )
    ...
    def get_subtask_term_signals(self):
        ...
        # )
        signals["open"] = int(self.env._check_success())
        return signals
